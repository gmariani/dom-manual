...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saxon="http://sf.net/saxon" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ifn="urn:internal:functions"
	...
	<xsl:param name="isStandalone" select="true()"/>
	...
	<xsl:template name="version">
		...
		<xsl:if test="not($config/options/@showVersions) or $config/options[@showVersions!='false']">
			<xsl:if test="count(prolog/asMetadata/apiVersion[not(starts-with(./apiLanguage/@version,'1'))]) or count(./prolog/asMetadata/apiVersion/apiPlatform/@version)">
						...
						<tr>
							...
							<td valign="top">
								<xsl:for-each select="prolog/asMetadata/apiVersion/apiTool">
									...
									<xsl:variable name="tokenizedVersion" select="tokenize(@version,'_')"/>
									
									<xsl:variable name="Version">
										<xsl:choose>
											<!-- For versions like '8.0 as IE7' -->
											<xsl:when test="$tokenizedVersion[2] = 'as'">
												<xsl:value-of select="string-join(($tokenizedVersion[1], translate($tokenizedVersion[2],$upperCase,$lowerCase), translate($tokenizedVersion[3],$lowerCase,$upperCase)), ' ')"/>
											</xsl:when>
											
											<!-- For versions like '8.0' -->
											<xsl:otherwise>
												<xsl:value-of select="$tokenizedVersion[1]"/>
											</xsl:otherwise>
										</xsl:choose>
									</xsl:variable>
									
									<xsl:variable name="CompatType">
										<xsl:choose>
											<!-- For versions like '8.0 as IE7' -->
											<xsl:when test="$tokenizedVersion[2]='as' and string-length($tokenizedVersion[4]) &gt; 0">
												<xsl:value-of select="$tokenizedVersion[4]"/>
											</xsl:when>
											
											<!-- For versions like '8.0' -->
											<xsl:when test="not($tokenizedVersion[2]='as') and string-length($tokenizedVersion[2]) &gt; 0">
												<xsl:value-of select="$tokenizedVersion[2]"/>
											</xsl:when>
									...
									<xsl:variable name="CapitalCompatType">
										<xsl:value-of select="concat(translate(substring($CompatType,1,1),$lowerCase,$upperCase),substring($CompatType,2))"/>
									</xsl:variable>
									
									<span title="Supported: {$CapitalCompatType}" class="comp {$CompatType}">
									
									<xsl:choose>
										<xsl:when test="$toolName='IE'">
											<img src="{$baseRef}images/IEIcon.png" width="16" height="16" hspace="0" vspace="0" alt="Internet Explorer" title="Internet Explorer"/>
										</xsl:when>
										<xsl:when test="$toolName='Firefox'">
											<img src="{$baseRef}images/FirefoxIcon.png" width="16" height="16" hspace="0" vspace="0" alt="Firefox" title="Firefox"/>
										</xsl:when>
										<xsl:when test="$toolName='Opera'">
											<img src="{$baseRef}images/OperaIcon.png" width="16" height="16" hspace="0" vspace="0" alt="Opera" title="Opera"/>
										</xsl:when>
										<xsl:when test="$toolName='Safari'">
											<img src="{$baseRef}images/SafariIcon.png" width="16" height="16" hspace="0" vspace="0" alt="Safari" title="Safari"/>
										</xsl:when>
										<xsl:when test="$toolName='Chrome'">
											<img src="{$baseRef}images/ChromeIcon.png" width="16" height="16" hspace="0" vspace="0" alt="Chrome" title="Chrome"/>
										</xsl:when>
									</xsl:choose>
				...
				<!-- Since -->
				<!-- Don't show Since information if it's non-standard. Kinda pointless ->
				<xsl:if test="count(prolog/asMetadata/apiVersion/apiSince) and prolog/asMetadata/apiVersion/apiPlatform[@name!='Non-Standard']">
					<table cellpadding="0" cellspacing="0" border="0">
						<tr>
							<td style="white-space:nowrap" valign="top">
								<b>
							...
							<td>
								<xsl:call-template name="insertIcon">
									<xsl:with-param name="apiPlatform" select="prolog/asCustoms/introduced"/>
								</xsl:call-template>
							</td>
						</tr>
					</table>
				...
				<xsl:if test="count(prolog/asMetadata/apiVersion/apiSince) and prolog/asMetadata/apiVersion/apiPlatform[@name!='Non-Standard']">
					<table cellpadding="0" cellspacing="0" border="0">
						<tr>
							<td class="sinceTableLabel">
								<b>
									<xsl:call-template name="getLocalizedString">
										<xsl:with-param name="key">Introduced in</xsl:with-param>
									</xsl:call-template>
									<xsl:text disable-output-escaping="yes">:&amp;nbsp;</xsl:text>
								</b>
							</td>
							<td>
								<xsl:call-template name="insertIcon">
									<xsl:with-param name="apiPlatform" select="prolog/asMetadata/apiVersion/apiSince"/>
								</xsl:call-template>
							</td>
						</tr>
						
						<xsl:if test="count(prolog/asCustoms/modified)">
							<tr>
								<td class="sinceTableLabel">
									<b>
										<xsl:call-template name="getLocalizedString">
											<xsl:with-param name="key">Modified in</xsl:with-param>
										</xsl:call-template>
										<xsl:text disable-output-escaping="yes">:&amp;nbsp;</xsl:text>
									</b>
								</td>
								<td>
									<xsl:call-template name="insertIcon">
										<xsl:with-param name="apiPlatform" select="prolog/asCustoms/modified"/>
									</xsl:call-template>
								</td>
							</tr>
						</xsl:if>
		...
		<xsl:if test="not($config/options/@showVersions) or $config/options[@showVersions!='false'] or $config/options[@showRuntimeVersions='true']">
			<!-- Multiple Versions -->
			<!-- Don't show Runtime Versions information if it's non-standard. Kinda pointless -->
			<!-- Hide Runtime Versions since it's being displayed by the icon or banner ->
			<xsl:if test="prolog/asMetadata/apiVersion[apiPlatform] and prolog/asMetadata/apiVersion/apiPlatform[@name!='Non-Standard']">
				<table cellpadding="0" cellspacing="0" border="0">
					<tr>
						<td style="white-space:nowrap" valign="top">
							<b>
						...
						<td>
							<xsl:for-each select="prolog/asMetadata/apiVersion/apiPlatform">
								<xsl:choose>
									<xsl:when test="@name='DOMCore'">
										<xsl:text>DOM Core </xsl:text>
									</xsl:when>
									<xsl:when test="@name='Non-Standard'">
					...
					<xsl:if test="string-length(@href)">
						<a href="{@href}">
							...
							<xsl:attribute name="rel">
								<xsl:if test="starts-with(@href,'http:')">
									<xsl:text>external nofollow</xsl:text>
								</xsl:if>
							</xsl:attribute>
	...
	<xsl:template name="insertIcon">
		<xsl:param name="apiPlatform"/>
		
		<xsl:variable name="name">
			<xsl:choose>
				<xsl:when test="string-length($apiPlatform[1]/@name) = 0">
					<xsl:choose>
						<xsl:when test="string-length($apiPlatform[1]/@version) = 0">
							<xsl:value-of select="tokenize($apiPlatform,'\s+')[1]"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="tokenize($apiPlatform[1]/@version,'\s+')[1]"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="$apiPlatform[1]/@name"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:variable name="version">
			<xsl:choose>
				<xsl:when test="string-length($apiPlatform[1]/@name) = 0">
					<xsl:choose>
						<xsl:when test="string-length($apiPlatform[1]/@version) = 0">
							<xsl:value-of select="tokenize($apiPlatform,'\s+')[2]"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="tokenize($apiPlatform[1]/@version,'\s+')[2]"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="$apiPlatform[1]/@version"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:choose>
			<xsl:when test="$name = 'Non-Standard'">
				<span class="nsIcon">Non-Standard</span>
			</xsl:when>
			
			<xsl:when test="$name = 'HTML'">
				<span class="domIcon">HTML <xsl:value-of select="$version"/></span>
			</xsl:when>
			
			<!-- Other -->
			<xsl:when test="$name = 'WebStorage'">
				<span class="domIcon">Web Storage</span>
			</xsl:when>
			
			<xsl:when test="$name = 'Selectors'">
				<span class="domIcon">Selectors API</span>
			</xsl:when>
			
			<xsl:when test="$name = 'ElemTrav'">
				<span class="domIcon">Element Traversal</span>
			</xsl:when>
			
			<xsl:when test="$name = 'CSSOMView'">
				<span class="domIcon"><abbr title="Cascading Style Sheets Object Model">CSSOM</abbr><xsl:value-of select="$nbsp"/>View</span>
			</xsl:when>
			
			<xsl:when test="$name = 'CSSOM'">
				<span class="domIcon"><abbr title="Cascading Style Sheets Object Model">CSSOM</abbr></span>
			</xsl:when>
			
			<!-- DOM Level 1 -->
			<xsl:when test="$name = 'DOMHTML'">
				<span class="domIcon"><abbr title="Document Object Model">DOM</abbr><xsl:value-of select="$nbsp"/><xsl:value-of select="$version"/><xsl:value-of select="$nbsp"/><abbr title="Hypertext Mark-up Language">HTML</abbr></span>
			</xsl:when>
			
			<!-- DOM Level 2 -->
				<xsl:when test="$name = 'DOMViews'">
				<span class="domIcon"><abbr title="Document Object Model">DOM</abbr><xsl:value-of select="$nbsp"/><xsl:value-of select="$version"/> Views</span>
			</xsl:when>
			
			<xsl:when test="$name = 'DOMStyle'">
				<span class="domIcon"><abbr title="Document Object Model">DOM</abbr><xsl:value-of select="$nbsp"/><xsl:value-of select="$version"/> Style</span>
			</xsl:when>
			
			<xsl:when test="$name = 'DOMTR'">
				<span class="domIcon"><abbr title="Document Object Model">DOM</abbr><xsl:value-of select="$nbsp"/><xsl:value-of select="$version"/><xsl:value-of select="$nbsp"/><abbr title="Traversal and Range">TR</abbr></span>
			</xsl:when>
			
			<!-- DOM Level 3 -->
			<xsl:when test="$name = 'DOMLS'">
				<span class="domIcon"><abbr title="Document Object Model">DOM</abbr><xsl:value-of select="$nbsp"/><xsl:value-of select="$version"/> Load and Save</span>
			</xsl:when>
			
			<xsl:when test="$name = 'DOMVAL'">
				<span class="domIcon"><abbr title="Document Object Model">DOM</abbr><xsl:value-of select="$nbsp"/><xsl:value-of select="$version"/> Validation</span>
			</xsl:when>
			
			<xsl:when test="$name = 'DOMXPath'">
				<span class="domIcon"><abbr title="Document Object Model">DOM</abbr><xsl:value-of select="$nbsp"/><xsl:value-of select="$version"/> XPath</span>
			</xsl:when>
			
			<!-- DOM Level All -->
			<xsl:when test="$name = 'DOMEvents'">
				<span class="domIcon"><abbr title="Document Object Model">DOM</abbr><xsl:value-of select="$nbsp"/><xsl:value-of select="$version"/> Events</span>
			</xsl:when>
			
			<xsl:when test="$name = 'DOMCore'">
				<xsl:if test="$version = '0'">
					<span class="domIcon"><abbr title="Document Object Model">DOM</abbr> 0</span>
				</xsl:if>
				<xsl:if test="$version = '1'">
					<span class="domIcon"><abbr title="Document Object Model">DOM</abbr> 1 Core</span>
				</xsl:if>
				<xsl:if test="$version = '2'">
					<span class="domIcon"><abbr title="Document Object Model">DOM</abbr> 2 Core</span>
				</xsl:if>
				<xsl:if test="$version = '3'">
					<span class="domIcon"><abbr title="Document Object Model">DOM</abbr> 3 Core</span>
				</xsl:if>
			</xsl:when>
		</xsl:choose>
		...
		<xsl:if test="$apiPlatform/@name = 'Deprecated'">
			<span class="depIcon">Deprecated</span>
			<xsl:value-of select="$nbsp"/>
		</xsl:if>
		
		<xsl:if test="$apiPlatform/@name = 'Obsolete'">
			<span class="obsIcon">Obsolete</span>
			<xsl:value-of select="$nbsp"/>
		</xsl:if>
	</xsl:template>
	
	<!-- Inserts icons but filters out Deprecated, Obsolete and Non-Standard -->
	<xsl:template name="insertSpecIcon">
		<xsl:param name="apiPlatform"/>
		<xsl:if test="$apiPlatform[@name!='Non-Standard'] and $apiPlatform[@name!='Deprecated'] and $apiPlatform[@name!='Obsolete']">
			<xsl:call-template name="insertIcon">
				<xsl:with-param name="apiPlatform" select="$apiPlatform"/>
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
