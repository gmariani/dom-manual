...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saxon="http://sf.net/saxon" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ifn="urn:internal:functions"
			...
			<div class="summarySection">
				...
				<table cellspacing="0" cellpadding="3" class="summaryTable {$tableStyle}" id="{$tableId}">
					...
					<xsl:for-each
						...
						<!-- Color Row BG if Deprecated or Non-Standard -->
						<xsl:variable name="bgColor">
							<xsl:choose>
								<xsl:when test="prolog/asMetadata/apiVersion/apiPlatform[@name='Deprecated']">
									<xsl:text>#BBBBFF</xsl:text>
								</xsl:when>
								<xsl:when test="prolog/asMetadata/apiVersion/apiPlatform[@name='Obsolete']">
									<xsl:text>#FFCACA</xsl:text>
								</xsl:when>
								<xsl:when test="prolog/asMetadata/apiVersion/apiPlatform[@name='Non-Standard']">
									<xsl:text>#FEE3BC</xsl:text>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text></xsl:text>
								</xsl:otherwise>
						...
						<tr class="{$rowStyle}" bgColor="{$bgColor}">
										...
										<xsl:choose>
											...
											<xsl:otherwise>
												<xsl:call-template name="insertIcon">
													<xsl:with-param name="apiPlatform" select="prolog/asMetadata/apiVersion/apiPlatform"/>
												</xsl:call-template>
											</xsl:otherwise>
										</xsl:choose>
										<xsl:if test="$hLink!=''">
					...
					<tr>
						<td class="detailHeaderName">
							<xsl:choose>
								<xsl:when test="$prog_language_name='javascript'"/>
								<xsl:otherwise>
									<xsl:call-template name="insertSpecIcon">
										<xsl:with-param name="apiPlatform" select="prolog/asMetadata/apiVersion/apiPlatform"/>
									</xsl:call-template>
								</xsl:otherwise>
							</xsl:choose>
	...
	<xsl:template match="apiOperation" mode="detail">
		...
		<div class="detailBody">
			...
			<!-- Show products versions, since, runtime versions -->
			<xsl:call-template name="version">
				<xsl:with-param name="baseRef" select="$baseRef"/>
			</xsl:call-template>
	...
	<xsl:template name="description">
		...
		<!-- Traces out all custom tags except for common ones
		<xsl:if test="string-length(prolog/asCustoms) &gt; 0">
			<xsl:for-each select="prolog/asCustoms">
				<xsl:for-each select="./*">
					<xsl:if test="name(.) !='internal' and name(.) !='exampleText' and name(.) !='media' and name(.) !='import' and name(.) !='page'">
						<xsl:message>~~~~~~ <xsl:value-of select="$sourceName"/></xsl:message>
		...
		<!-- <xsl:if test="string-length(prolog/asMetadata) &gt; 0">
			<xsl:for-each select="prolog/asMetadata">
				<xsl:for-each select="./*">
					<xsl:if test="name(.) !='internal' and name(.) !='exampleText' and name(.) !='media' and name(.) !='import' and name(.) !='page'">
						<xsl:message>~~~~~~ <xsl:value-of select="$sourceName"/></xsl:message>
						<xsl:message>~~~~~~ <xsl:value-of select="name(.)"/> : <xsl:value-of select="."/></xsl:message>
		...
		<!-- Show Non-Standard Banner -->
		<xsl:if test="prolog/asMetadata/apiVersion/apiPlatform[@name='Non-Standard']">
			<div style="border: 1px solid rgb(255, 183, 82); margin: 10px 0px; padding: 0px 10px; background-color: rgb(254, 227, 188); font-weight: bold; text-align: center;">
				<p style="margin: 4px 0px;">Non-standard <xsl:if test="prolog/asMetadata/apiVersion/apiPlatform[@name = 'Non-Standard' and @version != '0']">(<xsl:value-of select="prolog/asMetadata/apiVersion/apiPlatform[@name='Non-Standard']/@version"/>)</xsl:if></p>
			</div>
		</xsl:if>
		
		<!-- Show Deprecated Banner -->
		<xsl:if test="prolog/asMetadata/apiVersion/apiPlatform[@name='Deprecated']">
			<div style="border: 1px solid #5151FF; margin: 10px 0px; padding: 0px 10px; background-color: #B9B9FF; font-weight: bold; text-align: center;">
				<p style="margin: 4px 0px;">
					<xsl:text>Deprecated</xsl:text>
					<xsl:if test="string-length(prolog/asCustoms/as-of)">
						<xsl:text> since </xsl:text>
						<xsl:value-of select="normalize-space(prolog/asCustoms/as-of)"/>
					</xsl:if>
				</p>
			</div>
		</xsl:if>
		
		<!-- Show Obsolete Banner -->
		<xsl:if test="prolog/asMetadata/apiVersion/apiPlatform[@name='Obsolete']">
			<div style="border: 1px solid #AC6262; margin: 10px 0px; padding: 0px 10px; background-color: #FFDBDB; font-weight: bold; text-align: center;">
				<p style="margin: 4px 0px;">
